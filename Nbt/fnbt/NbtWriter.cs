...
public sealed class NbtWriter
    {
        ...
        private void GoDown(NbtTagType thisType)
            {
                if (nodes == null) nodes = new Stack<NbtWriterNode>();
                NbtWriterNode newNode = new()
                    {
                ...
                if (parentType == NbtTagType.List)
                    {
                        ...
                        if (listType != desiredType)
                            throw new NbtFormatException("Unexpected tag type (expected: " + listType + ", given: " +
                                                         desiredType);
                        if (listIndex >= listSize) throw new NbtFormatException("Given list size exceeded.");
                        listIndex++;
                    }
                else if (name == null)
                    {
        ...
        private static void CheckArray([NotNull] Array data, int offset, int count)
            {
                ...
                if (data.Length - offset < count)
                    throw new ArgumentException(
                        "count may not be greater than offset subtracted from the array length.");
            }
        ...
        public void BeginList(NbtTagType elementType, int size)
            {
                if (size < 0) throw new ArgumentOutOfRangeException("size", "List size may not be negative.");
        ...
        public void WriteIntArray([NotNull] int[] data, int offset, int count)
            {
                ...
                writer.Write(count);
                for (int i = offset; i < count; i++) writer.Write(data[i]);
            }
