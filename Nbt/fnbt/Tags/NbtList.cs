...
public sealed class NbtList : NbtTag, IList<NbtTag>, IList
    {
        ...
        public NbtList([NotNull] NbtList other)
            {
                ...
                foreach (NbtTag tag in other.tags) tags.Add((NbtTag)tag.Clone());
            }
                ...
                set
                    {
                        ...
                        else if (value < NbtTagType.Byte ||
                                 (value > NbtTagType.IntArray && value != NbtTagType.Unknown))
                            {
        ...
        public T[] ToArray<T>() where T : NbtTag
            {
                ...
                for (int i = 0; i < result.Length; i++) result[i] = (T)tags[i];
                return result;
            }
                ...
                for (int i = 0; i < length; i++)
                    {
                        ...
                        newTag.Parent = this;
                        if (newTag.ReadTag(readStream)) tags.Add(newTag);
                    }
        ...
        public void Insert(int tagIndex, [NotNull] NbtTag newTag)
            {
                ...
                if (listType == NbtTagType.Unknown) listType = newTag.TagType;
                newTag.Parent = this;
            }
        ...
        public void Clear()
            {
                for (int i = 0; i < tags.Count; i++) tags[i].Parent = null;
                tags.Clear();
            }
        ...
        void ICollection.CopyTo(Array array, int index)
            {
                ...
                NbtTag[] values = tags.ToArray();
                for (int i = 0; i < tags.Count; i++) array.SetValue(values[index + i], i);
            }
