        ...
        public NbtCompound([NotNull] NbtCompound other)
            {
                ...
                name = other.name;
                foreach (NbtTag tag in other.tags.Values) Add((NbtTag)tag.Clone());
            }
        ...
        public T Get<T>([NotNull] string tagName) where T : NbtTag
            {
                ...
                NbtTag result;
                if (tags.TryGetValue(tagName, out result)) return (T)result;
                return null;
            }
        ...
        public bool Remove([NotNull] string tagName)
            {
                if (tagName == null) throw new ArgumentNullException("tagName");
                NbtTag tag;
                if (!tags.TryGetValue(tagName, out tag)) return false;
                tags.Remove(tagName);
                tag.Parent = null;
                return true;
            }
        ...
        void ICollection.CopyTo(Array array, int index)
            {
                ...
                NbtTag[] values = tags.Values.ToArray();
