...
public sealed class NbtFile
    {
        ...
        [NotNull] private NbtTag rootTag;
                ...
                switch (compression)
                    {
                        ...
                        case NbtCompression.ZLib:
                            ...
                            stream.ReadByte();
                            using (DeflateStream decStream = new(stream, CompressionMode.Decompress, true))
                                {
                                    if (bufferSize > 0)
                                        return GetRootNameInternal(new BufferedStream(decStream, bufferSize),
                                            bigEndian);
        ...
        private static string GetRootNameInternal([NotNull] Stream stream, bool bigEndian)
            {
                ...
                NbtBinaryReader reader = new(stream, bigEndian);
        ...
        public long LoadFromFile([NotNull] string fileName, NbtCompression compression,
            [CanBeNull] TagSelector selector)
            {
                ...
                using (
                    FileStream readFileStream = new(fileName,
                        FileMode.Open,
                        FileAccess.Read,
                        FileShare.Read,
        ...
        public long LoadFromBuffer([NotNull] byte[] buffer, int index, int length, NbtCompression compression,
            [CanBeNull] TagSelector selector)
            {
                ...
                using (MemoryStream ms = new(buffer, index, length))
                    {
        ...
        public long SaveToFile([NotNull] string fileName, NbtCompression compression)
            {
                ...
                using (
                    FileStream saveFile = new(fileName,
                        FileMode.Create,
                        FileAccess.Write,
                        FileShare.None,
                        FileStreamBufferSize,
        ...
        public long SaveToBuffer([NotNull] byte[] buffer, int index, NbtCompression compression)
            {
                ...
                using (MemoryStream ms = new(buffer, index, buffer.Length - index))
                    {
                ...
                switch (compression)
                    {
                        case NbtCompression.ZLib:
                            ...
                            int checksum;
                            using (ZLibStream compressStream = new(stream, CompressionMode.Compress, true))
                                {
                                    BufferedStream bufferedStream = new(compressStream, WriteBufferSize);
                        ...
                        case NbtCompression.GZip:
                            using (GZipStream compressStream = new(stream, CompressionMode.Compress, true))
                                {
                        ...
                        case NbtCompression.None:
                            NbtBinaryWriter writer = new(stream, BigEndian) { UseVarInt = UseVarInt };
